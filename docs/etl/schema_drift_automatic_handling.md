...
> **Status (2026-06-26):** **Not implemented** — refactor plan only. Tiered runtime DDL evolution (`SchemaComparator`, Tier 1 `ALTER TABLE`, Tier 2 + tracking reset) has not been built. Partial mitigations exist today (see [§ Status](#status-2026-06)). Config-layer drift is handled separately by [update-schema](etl_schema_update_command_feature.md) / [SCD analyzer](schema_analysis_scd_improvements.md). Backlog: [TODO.md — Runtime schema drift](../../TODO.md#runtime-schema-drift-automatic-handling--not-started).
...
### Current Behavior (Partial — gaps remain)

When schema changes are detected at load time:

1. **Detection**: `PostgresSchema.verify_schema()` compares PostgreSQL columns to MySQL (count, names, types) during `ensure_table_exists()` in `postgres_loader.py`
2. **Attempted fix**: Logs warning *"recreating table with correct schema"* → `DROP TABLE … CASCADE` + recreate (`postgres_schema.py`)
3. **Gaps vs this plan**:
   - No change **classification** — every mismatch triggers full recreate (no Tier 1 `ALTER TABLE`)
   - **`etl_load_status` not reset** on recreate — stale `last_primary_value` can persist
   - Table may end up empty or partially loaded until a full reload completes
   - Incremental queries may return 0 rows until loader stale-state fallback triggers full load (works sometimes, inefficient)
   - Breaking changes still require manual intervention when auto-recovery fails
...
- [ ] Implement DROP CASCADE logic for breaking changes (extend existing recreate path)
...
### Not implemented (this document)

| Planned component | Code today |
| --- | --- |
| `SchemaComparator` | ❌ Not found |
| Tier 1 `ALTER TABLE` for compatible changes | ❌ |
| Tier 2 + **`etl_load_status` reset** on recreate | ❌ Explicit manager |
| `etl_schema_changes` changelog table | ❌ |
| Schema change notifications | ❌ |

### Partial mitigations (shipped elsewhere)

| Capability | Where |
| --- | --- |
| Column verify + `DROP … CASCADE` recreate | `etl_pipeline/etl_pipeline/core/postgres_schema.py` — `ensure_table_exists()`, `verify_schema()` |
| Incremental → full load when stale | `etl_pipeline/etl_pipeline/loaders/postgres_loader.py` — stale-state fallback |
| **`tables.yml` config refresh** (different layer) | [etl_schema_update_command_feature.md](etl_schema_update_command_feature.md), [schema_analysis_scd_improvements.md](schema_analysis_scd_improvements.md) |
| Table **count** drift sanity check | `mdc etl invoke -- check-schema-drift` |

**Layer distinction:** Config drift (`tables.yml` vs MySQL) is largely handled by nightly `update-schema`. **This plan** covers **PostgreSQL `raw.*` DDL** vs MySQL at load time — still open.

### Related docs

- [etl_schema_update_command_feature.md](etl_schema_update_command_feature.md) — config refresh (shipped)
- [schema_analysis_scd_improvements.md](schema_analysis_scd_improvements.md) — analyzer SCD (shipped)
- [ETL_CDC_IMPLEMENTATION_AND_OPTIONS.md](ETL_CDC_IMPLEMENTATION_AND_OPTIONS.md) — schema vs row-level change handling
- [TODO.md — Runtime schema drift](../../TODO.md#runtime-schema-drift-automatic-handling--not-started)
...
**Date**: 2025-10-11 (plan); status updated 2026-06-26  
**Status**: **Not implemented** — refactor plan; see [§ Status](#status-2026-06)
...
*This document outlines the complete solution for automatic **runtime** schema drift handling in the ETL pipeline. Implementation should follow the phased rollout plan with comprehensive testing at each stage.*
