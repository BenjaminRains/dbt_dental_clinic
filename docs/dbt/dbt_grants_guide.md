...
Always test grant changes in local before production:

```bash
# Test in local
dbt run --target local --models marts.dim_patient

# Verify grants work
# ... run tests ...

# Deploy to clinic (AWS production)
dbt run --target clinic --models marts.dim_patient
```
