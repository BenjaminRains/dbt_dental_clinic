"""
Verification script for LOCAL API Key Authentication

⚠️  THIS SCRIPT TESTS THE LOCAL API (http://localhost:8000) ⚠️
...
Prerequisites:
    - LOCAL API server must be running (run 'api-run' in another terminal)
    - requests library must be installed (run: pip install requests)
    - .ssh/dbt-dental-clinic-api-key.pem file must exist in project root (local dev key)
...
# Get API key from .ssh/dbt-dental-clinic-api-key.pem file only
def get_api_key():
    """
    Get LOCAL API key from .ssh/dbt-dental-clinic-api-key.pem file in project root.
    
    Returns None if not found (test will show appropriate error).
    """
    # Read from .ssh/dbt-dental-clinic-api-key.pem file
    # __file__ is at api/tests/verify_api_local_authentication.py
    # Get absolute path to avoid issues with relative paths
    script_path = os.path.abspath(__file__)  # Full path to verify_api_local_authentication.py
    ...
    print(f"   🔍 Looking for LOCAL key file at: {pem_file}")
...
def verify_business_endpoints_with_key():
    ...
    if test_status == 401:
        ...
        print(f"   The API key being used doesn't match the backend's API_KEY")
        print(f"\n💡 To fix this:")
        print(f"   1. Check what API_KEY the backend is using:")
        print(f"      - Check backend logs when it starts")
        print(f"      - Or check api/.env file for API_KEY")
        print(f"      - Or set API_KEY before starting backend")
...
def main():
    ...
    print("=" * 70)
    print("LOCAL API KEY AUTHENTICATION VERIFICATION")
    print("Testing: http://localhost:8000")
    print("=" * 70)
    
    # Check if API is running
    print("\n🔍 Checking if LOCAL API server is running...")
    try:
        response = requests.get(f"{BASE_URL}/health", timeout=5)
        if response.status_code == 200:
            print(f"✅ LOCAL API server is running at {BASE_URL}")
        else:
            print(f"⚠️  API returned status {response.status_code}")
    except requests.exceptions.ConnectionError:
        print(f"❌ Cannot connect to {BASE_URL}")
        print("   Make sure the LOCAL API server is running:")
