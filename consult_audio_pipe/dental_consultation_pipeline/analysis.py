...
Handles analysis of clean transcripts using Claude AI and ChatGPT 5.
"""
...
# Import OpenAI client for ChatGPT 5
try:
...
LLM_CONFIG = {
    ...
    "openai_model": "gpt-4o",  # ChatGPT 4 Omni model
}
...
def extract_patient_name(filename):
    ...
    # Handle special cases like "Voice 250618_115741" -> "Voice Recording 250618_115741"
    if name.startswith('Voice '):
        # Keep the timestamp to make it unique
        name = "Voice Recording " + name[6:]  # Remove "Voice " prefix, keep the rest
    elif name == 'consult1':
        name = "Consult 1"
...
def call_anthropic_api_summary(transcript, patient_name):
    ...
    try:
        ...
        # Save the full JSON response for debugging
        JSON_DEBUG_DIR.mkdir(exist_ok=True)
        json_filename = f"{patient_name}_claude_summary_response.json"
        json_path = JSON_DEBUG_DIR / json_filename
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump(json_response, f, indent=2)
        print(f"📄 JSON response saved: {json_path}")
...
def call_openai_api_analysis(transcript, patient_name):
    """Generate analysis using OpenAI ChatGPT 5 API and save full JSON response."""
    ...
    try:
        ...
        # Save the full JSON response for debugging
        JSON_DEBUG_DIR.mkdir(exist_ok=True)
        json_filename = f"{patient_name}_chatgpt5_analysis_response.json"
        json_path = JSON_DEBUG_DIR / json_filename
        with open(json_path, 'w', encoding='utf-8') as f:
...
def call_openai_api_summary(transcript, patient_name):
    """Generate summary using OpenAI ChatGPT 5 API and save full JSON response."""
    if not LLM_CONFIG["openai_api_key"]:
        print("❌ OpenAI API key not found. Set OPENAI_API_KEY environment variable.")
        return None
    ...
    try:
        ...
        # Save the full JSON response for debugging
        JSON_DEBUG_DIR.mkdir(exist_ok=True)
        json_filename = f"{patient_name}_chatgpt5_summary_response.json"
        json_path = JSON_DEBUG_DIR / json_filename
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump(response.model_dump(), f, indent=2)
        print(f"📄 JSON response saved: {json_path}")
...
def generate_summary_with_llm(transcript, patient_name, model="claude"):
    """Generate clean summary using specified LLM model"""
    if model.lower() == "claude":
        print(f"📝 Generating summary using Anthropic Claude...")
        return call_anthropic_api_summary(transcript, patient_name)
    elif model.lower() in ["chatgpt", "chatgpt5", "gpt5", "gpt-5"]:
        print(f"📝 Generating summary using OpenAI ChatGPT 5...")
        return call_openai_api_summary(transcript, patient_name)
    else:
        print(f"❌ Unknown model: {model}. Using Claude as default.")
        return call_anthropic_api_summary(transcript, patient_name)

def generate_analysis_with_llm(transcript, patient_name, model="claude"):
    """Generate analysis using specified LLM model"""
    if model.lower() == "claude":
        print(f"🤖 Generating analysis using Anthropic Claude...")
        return call_anthropic_api_analysis(transcript, patient_name)
    elif model.lower() in ["chatgpt", "chatgpt5", "gpt5", "gpt-5"]:
        print(f"🤖 Generating analysis using OpenAI ChatGPT 5...")
        return call_openai_api_analysis(transcript, patient_name)
    else:
        print(f"❌ Unknown model: {model}. Using Claude as default.")
        return call_anthropic_api_analysis(transcript, patient_name)

def generate_summary_with_chatgpt5(transcript, patient_name):
    """Generate clean summary using OpenAI ChatGPT 5 API"""
    print(f"📝 Generating summary using OpenAI ChatGPT 5...")
    return call_openai_api_summary(transcript, patient_name)

def generate_analysis_with_chatgpt5(transcript, patient_name):
    """Generate analysis using OpenAI ChatGPT 5 API"""
    print(f"🤖 Generating analysis using OpenAI ChatGPT 5...")
    return call_openai_api_analysis(transcript, patient_name)

def save_summary_to_markdown(summary, patient_name, model="claude"):
    """Save summary to Markdown file"""
    if model.lower() in ["chatgpt", "chatgpt5", "gpt5", "gpt-5"]:
        output_dir = CHATGPT_OUTPUT_DIR
        output_filename = f"{patient_name}_summary.md"
        llm_name = "OpenAI ChatGPT 5"
    else:
    ...
    try:
        # Add header with LLM information
        header = f"# Generated by {llm_name}\n\n"
...
def save_analysis_to_markdown(analysis, patient_name, model="claude"):
...
def process_transcript_with_llm(clean_transcript_path, model="claude"):
    ...
    # Determine LLM name for display
    if model.lower() in ["chatgpt", "chatgpt5", "gpt5", "gpt-5"]:
        llm_display_name = "OpenAI ChatGPT 5"
    else:
        llm_display_name = "Anthropic Claude 3.5 Sonnet"
    
    print(f"🟢 Processing: {patient_name} with {llm_display_name}")
    ...
    summary = generate_summary_with_llm(transcript, patient_name, model)
    if summary:
        # Save as Markdown
        summary_md_path = save_summary_to_markdown(summary, patient_name, model)
        if summary_md_path:
    ...
    analysis = generate_analysis_with_llm(transcript, patient_name, model)
    if analysis:
        # Save as Markdown
        analysis_md_path = save_analysis_to_markdown(analysis, patient_name, model)
        if analysis_md_path:
...
def analyze_clean_transcripts(model="claude"):
    """Analyze clean transcripts that correspond to audio files in raw_audio directory"""
    # Get list of audio files in raw_audio directory
    raw_audio_dir = PROJECT_ROOT / "raw_audio"
    if not raw_audio_dir.exists():
        print("❌ Raw audio directory not found")
        return []
    ...
    print(f"🎵 Found {len(audio_files)} audio file(s) in raw_audio directory:")
    for audio_file in audio_files:
        print(f"  • {audio_file}")
    
    # Get clean transcript files
    ...
    for clean_transcript_file in clean_transcript_files:
        # Extract the base name (remove _clean.txt)
        base_name = clean_transcript_file.stem.replace('_clean', '')
        
        # Check if this transcript corresponds to an audio file
        if base_name in audio_files:
            patient_name = extract_patient_name(base_name)
            
            # Check if analysis files exist for the specified model
            if model.lower() in ["chatgpt", "chatgpt5", "gpt5", "gpt-5"]:
                analysis_md = CHATGPT_OUTPUT_DIR / f"{patient_name}_analysis.md"
                summary_md = CHATGPT_OUTPUT_DIR / f"{patient_name}_summary.md"
            else:
                analysis_md = CLAUDE_OUTPUT_DIR / f"{patient_name}_analysis.md"
                summary_md = CLAUDE_OUTPUT_DIR / f"{patient_name}_summary.md"
    ...
    if not new_clean_transcript_files:
        print("✅ No new clean transcripts to analyze (that correspond to raw_audio files)")
        return []
    
    print(f"🤖 Found {len(new_clean_transcript_files)} new clean transcript(s) to analyze with {model.upper()}:")
    for clean_transcript_file in new_clean_transcript_files:
        print(f"  • {clean_transcript_file.name}")
    
    processed_count = 0
    for clean_transcript_file in new_clean_transcript_files:
        if process_transcript_with_llm(clean_transcript_file, model):
            processed_count += 1
    
    if processed_count > 0:
        # Determine output directory and LLM name for display
        if model.lower() in ["chatgpt", "chatgpt5", "gpt5", "gpt-5"]:
            output_dir_display = CHATGPT_OUTPUT_DIR
            llm_display_name = "OpenAI ChatGPT 5"
        else:
            output_dir_display = CLAUDE_OUTPUT_DIR
            llm_display_name = "Anthropic Claude 3.5 Sonnet"
        
        print(f"\n🎉 Successfully processed {processed_count} transcript(s) with {llm_display_name}")
        print(f"📁 Check {output_dir_display} for summary and analysis files (.md)")
        print(f"   • Summary files: *_summary.md")
        print(f"   • Analysis files: *_analysis.md")
    else:
        print("\n❌ No transcripts were successfully processed")
    
    return new_clean_transcript_files

def analyze_clean_transcripts_with_both_models():
    """Analyze clean transcripts using both Claude and ChatGPT 5 models"""
    print("🤖 Running analysis with both Claude and ChatGPT 5 models...")
    
    # Process with Claude
    print("\n" + "="*50)
    print("🟢 Processing with Anthropic Claude...")
    print("="*50)
    claude_results = analyze_clean_transcripts("claude")
    
    # Process with ChatGPT 5
    print("\n" + "="*50)
    print("🔵 Processing with OpenAI ChatGPT 5...")
    print("="*50)
    chatgpt5_results = analyze_clean_transcripts("chatgpt5")
    
    print("\n" + "="*50)
    print("📊 Summary of both model runs:")
    print(f"  • Anthropic Claude 3.5 Sonnet processed: {len(claude_results)} files")
    print(f"  • OpenAI ChatGPT 5 processed: {len(chatgpt5_results)} files")
    print("="*50)
    
    return claude_results + chatgpt5_results
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "analyze":
            ...
            analyze_clean_transcripts(model)
        elif sys.argv[1] == "analyze-both":
            analyze_clean_transcripts_with_both_models()
        elif sys.argv[1] == "tokens":
            calculate_token_usage()
        else:
            print("Usage:")
            print("  python -m dental_consultation_pipeline.analysis analyze [model]  - Process transcripts with specified model")
            print("  python -m dental_consultation_pipeline.analysis analyze-both     - Process transcripts with both models")
            print("  python -m dental_consultation_pipeline.analysis tokens           - Show token usage")
            print("\nAvailable models:")
            print("  claude    - Anthropic Claude 3.5 Sonnet (default)")
            print("  chatgpt5  - OpenAI ChatGPT 5")
    else:
        print("Usage:")
        print("  python -m dental_consultation_pipeline.analysis analyze [model]  - Process transcripts with specified model")
        print("  python -m dental_consultation_pipeline.analysis analyze-both     - Process transcripts with both models")
