...
Write-Host "`nRunning dbt on EC2" -ForegroundColor Cyan
    ...
    if (Test-Path $credentialsFile) {
        ...
        if ($Clinic -and $credentials.backend_api.clinic_api.ec2.instance_id) {
            ...
            if ($Clinic -and -not $credentials.backend_api.clinic_api.ec2.instance_id) {
                Write-Host "WARNING: -Clinic set but backend_api.clinic_api.ec2.instance_id missing; using backend_api.ec2.instance_id" -ForegroundColor Yellow
            }
            Write-Host "Loaded instance ID from deployment_credentials.json: $InstanceId" -ForegroundColor Green
        }
    } else {
        Write-Host "ERROR: deployment_credentials.json not found and InstanceId not provided" -ForegroundColor Red
...
if ($refreshProjectMode) {
    ...
    $DbtArgs = @("run")
    Write-Host "WARNING: No dbt command specified, defaulting to 'dbt run'" -ForegroundColor Yellow
}
...
if ($fullRefreshMode -and ($dbtSubcommand -eq 'run' -or $dbtSubcommand -eq 'build')) {
    if ($DbtArgs -notcontains '--full-refresh') {
        ...
        Write-Host "FullRefresh: appended --full-refresh (incremental models fully rebuild)" -ForegroundColor Yellow
    }
}

# Build the dbt command
$dbtCommand = $DbtArgs -join " "
Write-Host "`ndbt command: dbt $dbtCommand" -ForegroundColor Cyan
...
try {
    ...
    $loadEnvHelper = "$remoteProjectRoot/scripts/ec2/load_api_env.sh"
    ...
    # Prefer load_api_env.sh (safe KEY=VALUE + CRLF strip). Fall back to setup_ec2_dbt_env.sh.
    $inner = "cd '$DbtPath' && if [ -f '$loadEnvHelper' ]; then . '$loadEnvHelper' && load_clinic_dbt_env; elif [ -f '$envScriptRepo' ]; then . '$envScriptRepo'; elif [ -f '$envScriptLegacy' ]; then . '$envScriptLegacy'; fi && export PATH=`$HOME/.local/bin:/usr/local/bin:`$PATH && if command -v pipenv >/dev/null 2>&1 && [ -f Pipfile ]; then exec pipenv run dbt $escapedCommand; elif command -v dbt >/dev/null 2>&1; then exec dbt $escapedCommand; elif [ -x `$HOME/.local/bin/dbt ]; then exec `$HOME/.local/bin/dbt $escapedCommand; elif python3 -m dbt --version >/dev/null 2>&1; then exec python3 -m dbt $escapedCommand; else echo 'dbt not found. On instance as ec2-user: python3 -m pip install --user dbt-postgres' >&2; exit 127; fi"
    ...
    # Pass via temp file — PowerShell strips quotes if --parameters gets an inline JSON string.
    $parameters = @{
        commands = @($bashCommand)
    }
    $commandJson = $parameters | ConvertTo-Json -Compress
    $paramsFile = Join-Path $env:TEMP ("ssm-dbt-params-{0}.json" -f [guid]::NewGuid().ToString("N"))
    [System.IO.File]::WriteAllText($paramsFile, $commandJson, [System.Text.UTF8Encoding]::new($false))
    ...
    try {
        ...
        if ($commandId -is [array]) { $commandId = ($commandId | Out-String).Trim() } else { $commandId = "$commandId".Trim() }
        if ($LASTEXITCODE -ne 0 -or -not $commandId -or $commandId -match 'Error|error|Exception') {
            Write-Host "ERROR: aws ssm send-command failed" -ForegroundColor Red
            Write-Host $commandId -ForegroundColor Red
            exit 1
        }
    } finally {
    ...
    if (-not $finalStatus) {
        Write-Host "ERROR: No SSM status (timeout or get-command-invocation failed)." -ForegroundColor Red
        exit 1
    }
    ...
    if ($finalStatus -eq 'Success' -and $responseCode -eq 0) {
        Write-Host "`ndbt command completed successfully!" -ForegroundColor Green
    } else {
        Write-Host "`ndbt command failed" -ForegroundColor Red
    ...
    Write-Host "`nError running dbt command: $_" -ForegroundColor Red
    exit 1
}
