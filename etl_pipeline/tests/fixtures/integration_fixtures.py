...
def reset_analytics_tables(test_raw_engine):
    ...
    with test_raw_engine.connect() as conn:
        ...
        try:
            conn.execute(text('SELECT COUNT(*) FROM raw.patient')).scalar()
            conn.execute(text('SELECT COUNT(*) FROM raw.appointment')).scalar()
            print("[pytest fixture] Before truncation - table counts checked")
        except Exception as e:
    ...
    with test_raw_engine.connect() as conn:
        try:
            conn.execute(text('SELECT COUNT(*) FROM raw.patient')).scalar()
            conn.execute(text('SELECT COUNT(*) FROM raw.appointment')).scalar()
            print("[pytest fixture] After truncation - table counts checked")
        except Exception as e:
