...
Output:
    ...
    - etl_pipeline/logs/schema_analysis/reports/schema_analysis_YYYYMMDD_HHMMSS.json
...
from etl_pipeline.config.paths import (
    ensure_dir,
    schema_analysis_backup_display_path,
    schema_analysis_backups_dir,
    schema_analysis_reports_dir,
    schema_analysis_session_logs_dir,
    warn_legacy_log_dirs,
)
...
def setup_logging():
    ...
    ensure_dir(schema_analysis_session_logs_dir())
    ensure_dir(schema_analysis_reports_dir())
...
class OpenDentalSchemaAnalyzer:
    ...
    def __init__(
        ...
        # Get source database name from connection config
        source_config = self.settings.get_source_connection_config()
        self.source_db = source_config.get('database')
        
        if not self.source_db:
            raise ValueError("Source database is not configured in settings")
    ...
    # =========================================================================
    # SECTION 3: DISCOVERY METHODS (Find things)
    # =========================================================================
    # Methods that discover what exists: tables, dbt models
    # =========================================================================
    ...
    def _query_table_metrics(self, table_name: str) -> Dict:
        """
        Single source of truth: Query all table metrics from information_schema.
        ...
        with create_connection_manager(self.source_engine) as conn_manager:
            ...
            if estimated_rows == 0 and data_length > 0:
                estimated_rows = max(1, int(data_length / 1024))  # Assume ~1KB per row
    ...
    def discover_dbt_models(self) -> Dict[str, List[str]]:
        ...
        try:
            models_dir = Path(self.dbt_project_root) / 'dbt_dental_models' / 'models'
    ...
    def find_incremental_columns(self, table_name: str, schema_info: Dict) -> List[str]:
        ...
        # Special case: Add primary key for auto-incrementing tables BEFORE limiting
        ...
        if primary_keys:
            ...
            # Add primary key at the beginning if it's auto-incrementing and not already included
            if ('bigint' in primary_key_type or 'int' in primary_key_type) and primary_key not in prioritized_columns:
                prioritized_columns.insert(0, primary_key)  # Add at the beginning for highest priority
                logger.debug(f"Added auto-incrementing primary key '{primary_key}' to incremental columns for {table_name}")
        
        # Limit to 3 most reliable columns AFTER adding primary key to avoid complexity
    ...
    # =========================================================================
    # Methods that calculate performance characteristics:
    # - get_table_performance_profile(): Main performance analysis
    # - _calculate_processing_priority(): Assign priority levels
    # - _estimate_processing_time(): Calculate expected duration
    # - _estimate_memory_requirements(): Calculate memory needs
    # =========================================================================
    ...
    # =========================================================================
    # Methods that detect schema changes over time (slowly changing dimensions):
    # - _generate_schema_hash(): Create hash fingerprint of schema
    # - compare_with_previous_schema(): Detect added/removed tables and columns
    # =========================================================================
    ...
    # =========================================================================
    # Methods that process multiple tables efficiently:
    # - get_batch_schema_info(): Batch schema queries
    # - get_batch_size_info(): Batch size queries
    # =========================================================================
    ...
    # =========================================================================
    # Methods that orchestrate the complete analysis and generate tables.yml:
    # - generate_complete_configuration(): Main orchestrator
    # - analyze_complete_schema(): Top-level coordinator with SCD detection
    # - _validate_extraction_strategy(): Validate strategy values
    # =========================================================================
    ...
    def analyze_complete_schema(self, output_dir: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            schema_analysis_logs = ensure_dir(schema_analysis_session_logs_dir())
            schema_analysis_reports = ensure_dir(schema_analysis_reports_dir())
            schema_backups = ensure_dir(schema_analysis_backups_dir())
    ...
    # =========================================================================
    # Methods that generate human-readable reports and summaries:
    # - _generate_detailed_analysis_report(): Comprehensive JSON report
    # - _generate_performance_summary(): Performance metrics report  
    # - _generate_schema_changelog(): SCD changelog in markdown
    # - _generate_summary_report(): Console summary
    # - _get_batch_size_range(): Helper for statistics
    # =========================================================================
...
"""
        ...
        # Save summary report
        report_path = ensure_dir(schema_analysis_reports_dir()) / f'performance_summary_{timestamp}.txt'
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write(report)
...
"""
        ...
        changelog += f"*Backup configuration: `{schema_analysis_backup_display_path(timestamp)}`*\n"
