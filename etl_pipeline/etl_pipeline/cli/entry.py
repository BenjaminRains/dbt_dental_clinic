...
This file still imports from the old elt_pipeline.py (line 58) and needs to be
connected to the new orchestration framework in main.py and the orchestration modules.
The new framework provides better error handling, metrics, and monitoring.
"""
import os
import sys
import argparse
...
def main():
    ...
    run_parser.add_argument('--force-full', action='store_true',
                           help='Force full extraction and load for all tables')
...
def run_etl_pipeline(args):
    ...
    try:
        from etl_pipeline.etl_pipeline.elt_pipeline import IntelligentELTPipeline
        import logging
