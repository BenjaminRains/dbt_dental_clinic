...
import { clearLegacyClinicRoleStorage, isClinicSite } from '../config/clinicSite';
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const [session, setSession] = useState<PortalSession | null>(() =>
        clinicSite ? readStoredSession() : null
    );
    const [authLoading, setAuthLoading] = useState(clinicSite && !!readStoredSession());
    ...
    const value = useMemo(
        () => ({
            session,
            isAuthenticated: clinicSite ? !!session && !authLoading : true,
            isClinicBuild: clinicSite,
            role,
            homePath,
        ...
        [session, clinicSite, role, homePath, login, logout, authLoading]
    );
