/**
 * Portfolio v3 ΓÇô Production system presentation per frontend/docs/PORTFOLIO_OVERHAUL_PLAN.md
 * Order: Hero ΓåÆ Production ΓåÆ Architecture ΓåÆ Production Evidence ΓåÆ dbt Lineage ΓåÆ
 * Synthetic Dashboards ΓåÆ GitHub/Code ΓåÆ ProblemΓåÆSystemΓåÆOutcome ΓåÆ Docs ΓåÆ Consult Audio ΓåÆ
 * Supporting Tools ΓåÆ Skills ΓåÆ Contact ΓåÆ Footer
 */
...
const ARCHITECTURE_CHART = `flowchart TD
    A["OpenDental MariaDB<br/>450+ source tables"] --> B["Python ETL / Schema Discovery<br/>ΓÇó SQLAlchemy-powered introspection<br/>ΓÇó Metadata-driven extraction<br/>ΓÇó Incremental loading logic<br/>ΓÇó Change tracking + validation"]
    B --> C["PostgreSQL Warehouse<br/>RAW ΓåÆ replicated source tables<br/>STAGING ΓåÆ typed & cleaned inputs<br/>INTERMEDIATE ΓåÆ business logic & joins<br/>MARTS ΓåÆ KPI & dimensional models"]
    C --> D["dbt Core<br/>150+ Models<br/>Tests, Docs, Lineage Graph"]
    D --> E["FastAPI Backend<br/>ΓÇó REST API for KPIs & queries<br/>ΓÇó Caching, pagination, auth<br/>ΓÇó Direct access to marts schema"]
    E --> F["React Analytics Dashboard<br/>ΓÇó Real-time KPI cards<br/>ΓÇó Production & collections charts<br/>ΓÇó Provider performance analytics<br/>ΓÇó State mgmt via Zustand"]
    F --> G["AWS Deployment Prod<br/>CloudFront CDN ΓåÆ React frontend<br/>S3 ΓåÆ static assets<br/>EC2/Docker ΓåÆ FastAPI backend<br/>RDS PostgreSQL ΓåÆ warehouse<br/>CI/CD ΓåÆ automated tests & deploy"]
    G --> H["Consult Audio Pipeline Parallel<br/>Whisper ΓåÆ Transcript ΓåÆ Cleaning<br/>ΓåÆ LLM Summary ΓåÆ Structured TSV/JSON<br/>optional warehouse load<br/>Enriches analytics with consultation insights"]
...
const PortfolioV3: React.FC = () => {
    return (
        <Box sx={{ minHeight: '100vh', bgcolor: 'background.default' }}>
            {/* Top ribbon ΓÇö grey box; same width container as hero */}
            <Box
                sx={{
                    position: 'sticky',
                    top: 0,
            ...
            {/* 1. Hero (┬º8) ΓÇö light blue box; same width and shape as ribbon */}
                    ...
                    <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, alignItems: { md: 'flex-start' }, justifyContent: 'space-between', gap: 3 }}>
                        <Box sx={{ flex: 1, minWidth: 0, textAlign: 'left' }}>
                            ...
                            <Typography variant="h5" component="h2" gutterBottom sx={{ mb: 2, fontSize: { xs: '1.25rem', md: '1.5rem' }, fontWeight: 600, color: '#01579b' }}>
                                Analytics Engineer ΓÇô Healthcare & Modern ELT
                            </Typography>
                            ...
                            <Typography variant="body2" sx={{ mb: 2, opacity: 0.95, color: '#0d47a1' }}>
                                Remote US ΓÇó rains.bp@gmail.com ΓÇó{' '}
            ...
            {/* 3. Platform Architecture (┬º1) + 4. Data Flow Example (┬º4) */}
                ...
                <Card elevation={2} sx={{ textAlign: 'left' }}>
                    <CardContent sx={{ textAlign: 'left' }}>
                        ...
                        <Box component="ul" sx={{ m: 0, pl: 2, '& li': { typography: 'body2', color: 'text.secondary', mb: 0.5 } }}>
                            <li>Metadata-driven MySQL ΓåÆ PostgreSQL replication</li>
                            <li>Incremental + backfill-safe ELT</li>
                            <li>dbt staging ΓåÆ intermediate ΓåÆ marts with tests</li>
                        ...
                        <Paper variant="outlined" sx={{ p: 2, bgcolor: 'grey.50', textAlign: 'left' }}>
                            ...
                            <Typography variant="body2" color="text.secondary" sx={{ fontFamily: 'monospace', fontSize: '0.875rem' }}>
                                OpenDental Procedures ΓåÆ stg_procedure ΓåÆ int_patient_procedure ΓåÆ mart_revenue ΓåÆ FastAPI /revenue-opportunities ΓåÆ Revenue Opportunity Dashboard
                            </Typography>
                        </Paper>
                    </CardContent>
                </Card>
                        ...
                        <Card variant="outlined" sx={{ height: '100%', textAlign: 'left' }}>
                            <CardContent sx={{ textAlign: 'left' }}>
                                ...
                                <Box component="ul" sx={{ m: 0, pl: 2, '& li': { typography: 'body2', color: 'text.secondary', mb: 0.5 } }}>
                                    ...
                                    <li>Three-phase ETL: MySQL ΓåÆ replication ΓåÆ PostgreSQL raw; full / incremental / incremental_chunked strategies</li>
                        ...
                        <Card variant="outlined" sx={{ height: '100%', textAlign: 'left' }}>
                            <CardContent sx={{ textAlign: 'left' }}>
                                ...
                                <Box component="ul" sx={{ m: 0, pl: 2, '& li': { typography: 'body2', color: 'text.secondary', mb: 0.5 } }}>
                                    ...
                                    <li>Cost optimization: EC2 scheduling (Lambda + EventBridge), region consolidationΓÇö73% cost reduction</li>
                                </Box>
                            </CardContent>
                        </Card>
                    </Grid>
            ...
            <Box id="code" sx={{ px: { xs: 2, sm: 3, md: 6, lg: 8 }, pb: 6, scrollMarginTop: 56 }}>
                ...
                <Grid container spacing={2} sx={{ justifyContent: 'flex-start' }}>
                    {[
                        ...
                        { name: 'dbt Project', href: 'https://dbtdentalclinic.com/dbt-docs/', icon: <Analytics />, desc: '150+ models, staging ΓåÆ intermediate ΓåÆ marts, tests, lineage.' },
            ...
            {/* 9. Project cards: Problem ΓåÆ System ΓåÆ Outcome (┬º5) */}
                        ...
                        <Grid item xs={12} sm={6} md={4} key={title}>
                            <Card elevation={1} sx={{ height: '100%', textAlign: 'left' }}>
                                <CardContent sx={{ textAlign: 'left' }}>
                                    ...
                                    <Link href={href} target="_blank" rel="noopener noreferrer">View ΓåÆ</Link>
                                </CardContent>
                            </Card>
                        </Grid>
                    ))}
            ...
            <Box sx={{ px: { xs: 2, sm: 3, md: 6, lg: 8 }, pb: 6 }}>
                ...
                <Grid container spacing={2} sx={{ mb: 4, justifyContent: 'flex-start' }}>
                    {[
                        ...
                        { step: '4. HTML Conversion', icon: <Code />, text: 'Markdown ΓåÆ responsive HTML reports.', chips: ['Markdown2'] },
