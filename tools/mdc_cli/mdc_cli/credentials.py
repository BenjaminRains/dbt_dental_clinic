...
def resolve_demo_frontend_config() -> FrontendDeployConfig:
    bucket = _norm(os.environ.get("FRONTEND_BUCKET_NAME"))
    dist_id = _norm(os.environ.get("FRONTEND_DIST_ID"))
    domain = _norm(os.environ.get("FRONTEND_DOMAIN"))

    creds = load_deployment_credentials()
    if creds:
        if not bucket:
            bucket = _norm(_dig(creds, "frontend", "s3_buckets", "frontend", "bucket_name"))
        if not dist_id:
            dist_id = _norm(_dig(creds, "frontend", "cloudfront", "distribution_id"))
        if not domain:
            fd = _norm(_dig(creds, "frontend", "domain"))
            if fd:
                domain = f"https://{fd}"

    if not bucket:
        raise ValueError(
            "FRONTEND_BUCKET_NAME not set. Set env var or "
            "deployment_credentials.json frontend.s3_buckets.frontend.bucket_name."
        )
    if not dist_id:
        raise ValueError(
            "FRONTEND_DIST_ID not set. Set env var or "
            "deployment_credentials.json frontend.cloudfront.distribution_id."
        )
